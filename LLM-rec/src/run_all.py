...
def run_b1_llm_mlp(cfg: dict, out_dir: Path) -> list[dict]:
    """Model B1: LLM-MLP (ID + item LLM emb + user LLM emb)."""
    print("\n" + "=" * 60)
    print("Model B1: LLM-MLP (ID + item LLM + user LLM)")
    print("=" * 60)
    ...
    # Load user LLM embeddings (optional)
    user_llm = None
    user_llm_path = EMB_DIR / "user_llm_embeddings.npy"
    if user_llm_path.exists():
        user_llm = load_item_llm_embeddings(str(user_llm_path))
        print(f"  Loaded user LLM embeddings: {user_llm.shape}")
    else:
        print("  user_llm_embeddings.npy not found — running item-LLM-only variant")
    ...
    return _run_mlp(
        model, train_loader, val_loader,
        train_cfg["num_epochs"], train_cfg["learning_rate"], device,
        out_dir / "checkpoints" / "b1_llm_mlp.pt",
        out_dir / "metrics" / "b1_llm_mlp.json",
    )


def run_transformer(model_key: str, use_llm: bool, use_user_llm: bool, cfg: dict, out_dir: Path) -> None:
    """Run a Transformer model via kuairec.train.main (subprocess)."""
    label = "B2: LLM-Transformer" if use_llm else "A2: ID-Transformer"
    print("\n" + "=" * 60)
    print(f"Model {label}")
    print("=" * 60)
...
def main(argv: list[str] | None = None) -> None:
    ...
    if run_all or "b1" in args.models:
        results["B1 LLM-MLP"] = run_b1_llm_mlp(cfg, out_dir)
    ...
    if run_all or "b2" in args.models:
        ...
        results["B2 LLM-Transformer"] = []  # metrics written by kuairec.train.main
