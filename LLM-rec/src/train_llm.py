...
def main(config_path: str = "/Users/tanushreenepal/Desktop/AdRec-GenAI/LLM-rec/config/base.yaml"):
    ...
    # Load LLM item embeddings (precomputed)
    # Example path: ../kuairec/embeddings/item_llm_embeddings.npy
    llm_emb_path = "../kuairec/embeddings/item_llm_embeddings.npy"
    item_llm = load_item_llm_embeddings(llm_emb_path)  # shape [n_items_llm, llm_dim]

    # Align embedding table length with the number of items in the interactions
    n_items = stats["n_items"]
    if item_llm.shape[0] < n_items:
        pad_rows = n_items - item_llm.shape[0]
        print(
            f"Warning: LLM embedding table has {item_llm.shape[0]} items, "
            f"but interactions use {n_items}. Padding {pad_rows} rows with zeros."
        )
        pad = torch.zeros((pad_rows, item_llm.shape[1]), dtype=item_llm.dtype)
        item_llm = torch.cat([item_llm, pad], dim=0)
    elif item_llm.shape[0] > n_items:
        extra = item_llm.shape[0] - n_items
        print(
            f"Warning: LLM embedding table has {item_llm.shape[0]} items, "
            f"but interactions use {n_items}. Truncating {extra} extra rows."
        )
        item_llm = item_llm[:n_items]

    # Now item_llm.shape[0] == n_items
    ...
    out_dir = Path("../LLM-rec/outputs")
    (out_dir / "metrics").mkdir(parents=True, exist_ok=True)
    (out_dir / "checkpoints").mkdir(parents=True, exist_ok=True)

    metrics_path = out_dir / "metrics" / "metrics_llm.json"
    ckpt_path    = out_dir / "checkpoints" / "checkpoints_llm.pt"
