...
class BaselineModel(torch.nn.Module):
    ...
    def __init__(self, user_num, item_num, feat_statistics, feat_types, args):  #
        ...
        self.temp = torch.nn.Parameter(torch.tensor(0.07))  # learnable temperature
    ...
    def forward(
        ...
        # Return embeddings + loss mask; InfoNCE will be computed outside.
    ...
    def compute_infonce_loss(self, seq_embs, pos_embs, neg_embs, loss_mask):
        ...
        neg_embedding_all = neg_embs.reshape(-1, hidden_size)                             # [B*L, H]
        neg_logits = torch.matmul(seq_embs, neg_embedding_all.transpose(-1, -2))          # [B, L, B*L]
    
        # concatenate [pos | negs]
        logits = torch.cat([pos_logits, neg_logits], dim=-1)                              # [B, L, 1+B*L]
    
        # keep only valid (next item) positions and apply temperature
        logits = logits[loss_mask.bool()] / self.temp                                     # [N_valid, 1+B*L]
